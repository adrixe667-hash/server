...
progress 10 "Verificando dependencias del sistema..."
scripts/check_dependencies.sh

progress 30 "Preparando archivo .env..."
if [[ ! -f .env ]]; then
  cp .env.example .env
  echo "Se creó .env desde .env.example."
fi

progress 50 "Validando nombre de servidor (MC_NAME)..."
...
progress 75 "Validando configuración Docker Compose..."
docker compose config >/dev/null

progress 100 "FINALIZADO: entorno listo para levantar"
